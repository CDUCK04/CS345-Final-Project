...
namespace DM
{
    public class ControlManager : MonoBehaviour, IDamagable
    {
        [Header("Initialize")]
        ...
        public static ControlManager Instance; //creates a definition for ControlManager instance;
        ...
        public void SwordComboAttack() {
            canAttack = false;
            isAttacking = true;
            anim.SetTrigger("combo");   //Set trigger named "combo" on
            float attackLength = GetAnimationLength("combo");
            StartCoroutine(ResetAttackCooldown(attackLength));
        }

        public void SwordNormalAttack() {
            canAttack = false;
            isAttacking = true;
            string targetAnim;
            ...
            StartCoroutine(ResetAttackCooldown(0.1f));
        }
        ...
        IEnumerator ResetAttackBool(float delay) {
            yield return new WaitForSeconds(delay);
            isAttacking = false;
        }
